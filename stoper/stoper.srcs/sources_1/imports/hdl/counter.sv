...
module counter
    ...
    logic [3:0] minute_value_nxt;
    
    always_ff @(posedge clk, posedge rst) begin : counter_minute
        if(rst) begin
            LED <= '0;
        end
        else begin
            LED <= minute_value_nxt;
        end
    end
    ...
    always_comb begin : minute_value_nxt_blk
        if(counter_value == 6000) begin
            if(LED == 10) begin
                minute_value_nxt = 0;
            end
            else begin
            minute_value_nxt = LED+1;
            end
        end
        else begin
            minute_value_nxt = LED;
        end
    end
